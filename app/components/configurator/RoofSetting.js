...
function RoofSetting({selectedOptions, setSelectedOptions}) { 
    ...
    const roofColor = [
      ...
      const handleSelectColor = (roofColor) => {
        setSelectedOptions({ ...selectedOptions, roofColor });
      };
    ...
    <div className='py-2'>
      ...
      <Grid item xs={12} className='pt-2'>
        <div className='flex flex-wrap gap-2 ' spacing={2}>
          {roofColor.map((color) => (
          <div className={`max-w-[80px] ${selectedOptions.roofColor===color.name ? ' font-bold' : null}`}><div className='w-20 h-12 rounded-md' style={{backgroundColor:color.ral}} onClick={() => handleSelectColor(color.name)}></div> <p className='text-xs text-center text-black'>{color.name}</p>       
          </div>
          ))}
        </div>
      </Grid>
