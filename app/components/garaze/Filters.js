...
const getData = async (filter) => {
  ...
  const response = await axios.get(
    ...
    {
      params: {
        per_page: 100, // Ilość wyników na stronę
        'acf.rodzaj': filter, // Filtrowanie wg acf.rodzaj
      },
    }
  );
...
function Filters() {
  ...
  }, [filter]); // Wywołanie useEffect przy zmianie filtra
        ...
        {types.map((item, index) => {
          return (
            <div
              ...
              className="relative border-2 transition-all group cursor-pointer md:w-[250px] md:h-[200px] w-[100px] h-[75pxpx]"
            >
              <img src={item.img} alt={item.title} className={`w-full h-full`} />
              <div
                className={`absolute top-0 left-0 w-full h-full bg-black bg-opacity-50 flex justify-center text-center items-end transition-all opacity-100 group-hover:bg-opacity-0 ${
